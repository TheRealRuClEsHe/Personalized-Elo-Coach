...
- **ID**: ISSUE-013
- **Date**: 2026-09-12
- **Problem**: Pushing local work to GitHub was blocked by a chain of git-state issues in the sandboxed remote-devices shell used to operate on the mounted repo folder. (1) The repo was left mid-merge (`origin/main` had 3 README-only commits the local branch didn't, local had 2 unrelated feature/hygiene commits) with the README conflict already resolved and staged, but the merge never finalized. (2) A stale `.git/index.lock` from a prior `git status` was left behind because that shell's `unlink` syscall on files under the mounted folder is deny-by-default, so git's own lock cleanup silently failed (`warning: unable to unlink ... Operation not permitted`), and the stale lock blocked further git commands. (3) The sandbox has no git identity configured (`user.name`/`user.email`, no `~/.gitconfig`), so `git commit` failed with "Author identity unknown" until set explicitly. (4) `git commit --no-edit` on the merge picked up the literal commit-template comment lines (`# Please enter a commit message...`) into the permanent commit message instead of stripping them. (5) The sandboxed shell has no GitHub credentials (no credential helper, no `gh` CLI), so `git push` failed with `fatal: could not read Username for 'https://github.com'` even after the merge was clean.
- **Proposed fix**: For (2), request delete permission on the connected folder via `device_request_delete_permission` before removing the stale lock, rather than assuming `rm` will work. For (3), set local (non-global) `user.name`/`user.email` matching the existing commit authors so the merge commit's authorship is consistent with the rest of the repo's history. For (4), amend the merge commit with an explicit `-m` message instead of trusting `--no-edit` against a template that wasn't cleaned. For (5), push must happen from the user's own authenticated terminal on the actual machine, not from the sandboxed bridge shell — surfaced this to the user instead of attempting a workaround (e.g. asking for a PAT) as the default path.
- **Method**: Called `device_request_delete_permission` for the repo's connected folder, then `rm -f .git/index.lock`. Ran `git commit --no-edit` to close the merge, found the comment lines had been captured, and fixed it with `git commit --amend -m "Merge remote-tracking branch 'origin/main'"`. Set `git config user.name "Hercules Li"` / `git config user.email "herculesli@Herculess-MacBook-Air.local"` locally (matching prior commit authors) before committing. Confirmed `git status` was clean and 3 commits ahead, then asked the user to run `git push origin main` themselves from their own Terminal rather than pushing from the sandbox.
- **Result**: RESOLVED — merge commit `d7cf107` completed with a clean message and correct authorship; user pushed successfully from their own terminal (`5877b22..d7cf107 main -> main`). Render should auto-deploy from the push if auto-deploy is enabled on the service; not independently verified this session.
- **Tags**: #git #merge #sandbox #permissions #credentials #tooling #deploy
...
- **ID**: ISSUE-014
- **Date**: 2026-09-12
- **Problem**: Changing the "Top Recommendations" number field (`#top-n`) after a report was already on screen had no effect — the recommendation list stayed frozen at whatever count was requested the last time "Analyze Replay" or the demo link was clicked. Root cause: `topN` was only ever read from the input at the moment those two buttons (plus `selectPlayer()`, the profile-picker re-submit path) fired their `fetch`/`XHR` call; no `input`/`change` listener existed on `#top-n` itself, so editing the number alone never triggered a re-render or a re-fetch. Confirmed the backend was not at fault: `GET /demo?top_n=8` against the live Render deploy correctly returned 8 recommendations sorted by priority — the bug was entirely client-side.
- **Proposed fix**: Rather than re-fetching on every keystroke (extra round-trips, spinner flicker), always request the maximum allowed count (`top_n=10`, the backend's `le=10` ceiling) from `/analyze` and `/demo`, cache that full response, and have `#top-n` re-render instantly from the cached array via `Array.prototype.slice`. Since the backend already sorts all candidate recommendations by `priority` descending before slicing (`src/coaching.py::rank_recommendations`), any prefix of the top-10 response is itself the correct top-N — no need to hit the network again when the count changes.
- **Method**: Edited `app/static/index.html` (all three fetch call sites — `analyzeBtn`, `selectPlayer()`, `demo-btn`) to request `top_n=${MAX_TOP_N}` (10) instead of the field's live value. Added `MAX_TOP_N`, a module-level `_lastFullData` cache, `clampTopN()` (parses and clamps to 1-10, falling back to 5 on garbage input), and `applyTopN()`, which re-renders `renderResults()` with `_lastFullData.recommendations.slice(0, topN)`. Wired `applyTopN` to `#top-n`'s `input` event so the list updates live as the number is typed/spun, with no network call. Updated the `replay_analyzed`/`demo_analyzed` analytics events to report the displayed count (`min(returned, requested)`) rather than the always-10 raw count. Edited in place on the user's machine via a Python read/replace/write script (file uses CRLF line endings throughout; preserved via `newline=''`) rather than retyping content, per this repo's edit conventions. Verified the edited `<script>` block still parses (`new Function(js)` in Node) and re-confirmed via `grep` that all three fetch sites now reference `MAX_TOP_N` and that `_lastFullData`/`applyTopN`/`clampTopN` are wired correctly. Did not run the Python test suite — this is a static-HTML/JS-only change with no pytest coverage over the frontend, and no working `.venv` was present in this sandboxed shell to run one anyway.
- **Result**: PENDING — logic and syntax verified statically; not yet exercised in an actual browser. User should reload the page, run "Try a demo" or upload a replay, then change the Top field and confirm the recommendation list grows/shrinks immediately without re-clicking anything, before this is committed and pushed.
- **Tags**: #frontend #ux #javascript #bug #caching #demo #analyze
